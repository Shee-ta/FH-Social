...
class Event {
  ...
  static List<Event> sortEventsByDate(List<Event> events) {
    events.sort((a, b) {
      final aNextOccurrence = a.days.isNotEmpty
          ? Formatter.calculateNextIso8601(a.iso8601startDateTime, a.days)
          : a.iso8601startDateTime;
      final bNextOccurrence = b.days.isNotEmpty
          ? Formatter.calculateNextIso8601(b.iso8601startDateTime, b.days)
          : b.iso8601startDateTime;
      return aNextOccurrence.compareTo(bNextOccurrence);
    });
    return events;
  }
  ...
  static String getStatus(Event event) {
    ...
    final nextDateTime = Formatter.iso8601StringToDateTime(nextIso).toLocal();
    DateTime? endTime;
    if (event.iso8601endDateTime.isNotEmpty) {
      final parsedEnd = DateTime.tryParse(event.iso8601endDateTime);
      if (parsedEnd != null) {
        ...
        if (endTime.isBefore(nextDateTime)) {
...
class EventDraft {
  ...
  void setEventDraft(EventDraft draft) {
    id = draft.id;
    title = draft.title;
  ...
  void reset() {
    ...
    description = '';
    recommendation = '';
    room = Locations.rooms.keys.first;
    coordinates = null;
    date = null;
    startTime = null;
    endTime = null;
